...
func NewFakeUdpConn(l *UdpListener, laddr, raddr net.Addr) *FakeUdpConn {
	fc := &FakeUdpConn{
		Logger:     log.NewPrefixLogger("FACKUDP"),
		l:          l,
		localAddr:  laddr,
		remoteAddr: raddr,
...
func ListenUDP(bindAddr string, bindPort int) (l *UdpListener, err error) {
	...
	l = &UdpListener{
		...
		Logger:    log.NewPrefixLogger("UDPLISTENER"),
	}
...
func ConnectUdpServer(addr string) (c Conn, err error) {
	servertAddr, err := net.ResolveUDPAddr("udp", addr)
	if err != nil {
		return
	}
	conn, err := net.DialUDP("udp", nil, servertAddr)
	if err != nil {
		return
	}
